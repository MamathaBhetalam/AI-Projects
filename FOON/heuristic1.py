...
# Function to perform a Greedy Best-First Search (GBFS)
...
def greed_bfs_heuristic1(candidate_units):
    """
    ...
    max_success = motion.get(foon_functional_units[candidate_units[0]].motion_node, 0)

    for candidate_unit in candidate_units:
        succes = motion.get(foon_functional_units[candidate_unit].motion_node, 0)
        if succes > max_success:
            c_unit = candidate_unit
            max_success = succes

    return c_unit

# The function `greed_bfs_heuristic1` is a heuristic function used for Greedy Best-First Search. It selects the candidate unit with the highest success rate based on a heuristic defined by the `motion` dictionary.
...
if __name__ == '__main__':
    ...
    for node in goal_nodes:
        ...
        for object1 in foon_object_nodes:
            if object1.check_object_equal(node_object):
                output_task_tree = search_GBFS(greed_bfs_heuristic1, kitchen_items, object1)
                save_paths_to_file(output_task_tree,
                                   'output_GBFS_heuristic1_{}.txt'.format(node["label"]))
                break
