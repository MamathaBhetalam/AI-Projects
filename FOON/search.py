...
def search_BFS(kitchen_items=[], goal_node=None):
    ...
    while len(items_to_search) > 0:
        ...
        if not check_if_exist_in_kitchen(kitchen_items, current_item):
            ...
            #check number of input_objects for each candidate and choose the one that has least number of inputs.

            #read motion nodes file and store in any type os data structure.
            # choose the candidate which has highest motion probability. 
